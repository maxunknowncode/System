...
export default {
  ...
  async execute(interaction) {
    ...
    if (!interaction.member.permissions?.has(PermissionsBitField.Flags.KickMembers)) {
      embed.setDescription(lang === 'de' ? 'Dir fehlt die Berechtigung KICK_MEMBERS.' : 'You lack the KICK_MEMBERS permission.');
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }
    ...
    if (targetUser.id === interaction.user.id) {
      embed.setDescription(lang === 'de' ? 'Du kannst dich nicht selbst kicken.' : 'You cannot kick yourself.');
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }

    if (targetUser.bot) {
      embed.setDescription(lang === 'de' ? 'Bots können nicht gekickt werden.' : 'Bots cannot be kicked.');
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }
