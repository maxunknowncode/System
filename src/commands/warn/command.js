...
export default {
  ...
  async execute(interaction) {
    ...
    if (targetUser.id === interaction.user.id) {
      embed.setDescription(lang === 'de' ? 'Du kannst dich nicht selbst verwarnen.' : 'You cannot warn yourself.');
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }

    if (targetUser.bot) {
      embed.setDescription(lang === 'de' ? 'Bots können nicht verwarnt werden.' : 'Bots cannot be warned.');
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }
