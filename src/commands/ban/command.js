...
export default {
  ...
  async execute(interaction) {
    ...
    if (targetUser.id === interaction.user.id) {
      embed.setDescription(lang === 'de' ? 'Du kannst dich nicht selbst bannen.' : 'You cannot ban yourself.');
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }

    if (targetUser.bot) {
      embed.setDescription(lang === 'de' ? 'Bots können nicht gebannt werden.' : 'Bots cannot be banned.');
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }
    ...
    if (hasTeamRole(targetMember)) {
      embed.setDescription(
        lang === 'de' ? 'Aktion blockiert: Ziel gehört zum Team.' : 'Action blocked: target is part of the team.'
      );
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }
    ...
    if (targetMember && !compareHierarchy(interaction.member, targetMember)) {
      embed.setDescription(
        lang === 'de' ? 'Rollen-Hierarchie verhindert diese Aktion.' : 'Role hierarchy prevents this action.'
      );
      await interaction.editReply({ embeds: [embed], components: [] });
      return;
    }
